...
.hero-sub { font-size: 0.78rem; color: var(--sub); letter-spacing: 0.06em; margin-bottom: 2rem; }

/* TABS */
.stTabs [data-baseweb="tab-list"] { gap:0; background:transparent; border-bottom:2px solid var(--border); }
.stTabs [data-baseweb="tab"] { font-size:0.86rem; font-weight:400; color:var(--sub); padding:0.6rem 1.4rem; border-radius:0; background:transparent; border:none; }
.stTabs [aria-selected="true"] { color:var(--text) !important; background:transparent !important; border-bottom:2.5px solid var(--accent) !important; font-weight:600 !important; }
...
.mini-dot { width:14px; height:14px; border-radius:50%; flex-shrink:0; border:1px solid rgba(0,0,0,0.1); }
...
.pcard-shade { font-size:0.76rem; color:var(--sub); }
...
/* RESULT BOX */
.rbox-warm { background:linear-gradient(135deg,#FFFBE0,#FFF3A0); border:1.5px solid #E8D040; border-radius:18px; padding:1.8rem 2rem; margin-bottom:1.2rem; }
.rbox-cool { background:linear-gradient(135deg,#F0F4FF,#E0EAFF); border:1.5px solid #B0C0E8; border-radius:18px; padding:1.8rem 2rem; margin-bottom:1.2rem; }
...
details summary { font-size:0.8rem !important; color:var(--sub) !important; }
.footer { text-align:center; font-size:0.72rem; color:#A09830; padding:1.5rem 0 2rem; }
</style>
...
DB = {
    "롬앤 쥬시 래스팅 틴트": {
        ...
        "28 핑크핑크":          dict(tone="cool",season=["summer"],           chroma=3,value=3,category="lip",hex="#E070A8",avail=True, note="발랄한 쿨핑크 — 2024",            new=True),
        "29 브릭베이지":        dict(tone="warm",season=["autumn"],           chroma=2,value=2,category="lip",hex="#A06850",avail=True, note="웜 브릭베이지 — 2024",            new=True),
    },
    "에뛰드 픽싱 틴트": {
...
# ══════════════════════════════════════════════════════════════
# 제품 선택 위젯
# - 제품명 selectbox: 선택 전 "제품명 선택" / 선택 후 제품명 표시
# - 호수 selectbox:   선택 전 "호수 선택"  / 선택 후 호수명 표시
# - 선택 후 컬러 점 + 태그 한 줄만 표시 (아이콘 없음)
# ══════════════════════════════════════════════════════════════
def product_picker(prefix, include_disc=False):
    ...
    st.markdown(
        ...
        f'{disc}{np} &nbsp;<span style="color:#B0A840">{d["note"]}</span>'
        f'</div>',
        unsafe_allow_html=True,
    )
    return (pname, shade)
...
<div class="pcard" style="flex-direction:column;align-items:flex-start;gap:0.28rem;">
    <div style="display:flex;align-items:center;gap:0.55rem">
        {dot_html(d['hex'],22)} <b style="font-size:0.83rem">{pn}</b>{'&nbsp;'+new_tag() if d.get('new') else ''}
    </div>
    <div style="padding-left:28px"><div class="pcard-shade">{sh}</div><div class="pcard-note">{d['note']}</div></div>
</div>""", unsafe_allow_html=True)
    st.divider()
    ...
    st.divider()
    tips={"spring":"봄 웜톤은 **밝고 산뜻한 코랄·피치 조합**이 최고예요.",
...
<div class="pcard" style="flex-direction:column;align-items:flex-start;gap:0.22rem;margin-bottom:0.4rem;">
    <div style="display:flex;align-items:center;gap:0.5rem">
        {dot_html(d['hex'],18)} <b style="font-size:0.8rem">{pn}</b>
    </div>
    <div style="padding-left:24px"><div style="font-size:0.75rem;color:#7A7230">{sh}</div>
...
<div class="pcard" style="flex-direction:column;align-items:flex-start;border-radius:12px;padding:0.9rem;gap:0.35rem;">
    <div style="display:flex;align-items:center;gap:0.6rem">
        {dot_html(d['hex'],22)}
        <div><div class="pcard-name">{pn}{disc} {new_tag() if d.get('new') else ''}</div>
        <div class="pcard-shade">{sh}</div></div>
    </div>
    <div class="pcard-note">{d['note']}</div>
...
<div style="text-align:center;padding:0.45rem 0.2rem;">
    <div style="width:30px;height:30px;border-radius:50%;background:{d['hex']};margin:0 auto 0.3rem;border:2px solid rgba(0,0,0,0.07);"></div>
    <div style="font-size:0.72rem;font-weight:500;line-height:1.3">{pn}</div>
    <div style="font-size:0.67rem;color:#A09830">{sh}</div>
</div>""", unsafe_allow_html=True)
...
<div class="hero-sub">퍼스널 컬러 기반 뷰티 큐레이터 · 세부 8톤 분석 · 국내 구매 가능 · 2024 신제품 포함</div>
""", unsafe_allow_html=True)
    ...
    st.markdown('<div class="footer">TONEME · 단종 제품은 추천 제외 · 국내 구매 가능 · 2024 신제품 포함</div>', unsafe_allow_html=True)
